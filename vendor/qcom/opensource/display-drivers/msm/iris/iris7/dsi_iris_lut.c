...
int iris_parse_lut_cmds_i7(uint32_t flag)
{
	...
	if (!ret) {
		...
		data[0].buf = fw->data;
		data[0].size = fw->size;
		IRIS_LOGI("%s(%d), load name: %s, size: %u.",
...
load_calibrated:
	...
	if (!ret) {
		...
		IRIS_LOGI("%s(%d), load calibrated fw, name: %s, size: %u.",
	...
	if (!ret) {
		...
		if (flag == LOAD_CALIBRATED_OR_GOLDEN) {
			IRIS_LOGE("%s(), failed to load calibrated FW: %s , goto load gelden", __func__, ccf2_name);
			goto load_golden;
		} else if (flag == LOAD_CALIBRATED_ONLY) {
	...
	if (!ret) {
		...
		if (flag == LOAD_CALIBRATED_OR_GOLDEN) {
			IRIS_LOGE("%s(), failed to load calibrated FW: %s , goto load gelden", __func__, ccf4_name);
			goto load_golden;
		} else if (flag == LOAD_CALIBRATED_ONLY) {
...
load_golden:
	...
	if (!ret) {
		...
		data[1].buf = ccf1_fw->data;
		data[1].size = ccf1_fw->size;
		IRIS_LOGI("%s(%d), load golden FW, name: %s, size: %u.",
			__func__, __LINE__, ccf1_name, data[1].size);
	} else {

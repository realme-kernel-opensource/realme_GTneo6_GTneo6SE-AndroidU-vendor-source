...
int iris_sdr_to_edr_nits(u32 nit, u32 curve_ratio)
{
	...
	case 3:		//3x
		if (sdr_nit <= 30 * 10000)
			edr_nit = sdr_nit * 2;
		else if (sdr_nit > 30 * 10000 && sdr_nit <= 150 * 10000)
			edr_nit = (sdr_nit * 13 - 150 * 10000) / 4;
		else if (sdr_nit > 150 * 10000 && sdr_nit <= 550 * 10000)
			edr_nit = (sdr_nit + 1650 * 10000) / 4;
		else
			edr_nit = sdr_nit;
		break;
	case 2:		//2.5x
		if (sdr_nit <= 30 * 10000)
			...
			edr_nit = (sdr_nit * 21 - 150 * 10000) / 8;
		else if (sdr_nit > 150 * 10000 && sdr_nit <= 550 * 10000)
			edr_nit = (sdr_nit * 7 + 4950 * 10000) / 16;
		else
			edr_nit = sdr_nit;
		break;
	case 1:
	default:	//2x
		if (sdr_nit <= 30 * 10000)
			...
			edr_nit = (sdr_nit * 5 + 1650 * 10000) / 8;
		else
			edr_nit = sdr_nit;
		break;
	}
...
int iris_EDR_backlight_ctrl_i7(u32 hdr_nit, u32 ratio_panel)
{
	...
	scale_max = iris_sdr_to_edr_nits(150, edr->curve_ratio) / 150;
	range = 2 - 1; // <30nit
	...
	if (hdr_nit < 550) {
		edr_final_nit = iris_sdr_to_edr_nits(sdr_nit, edr->curve_ratio);
		edr_final_ratio = edr_final_nit/sdr_nit;
		if (edr_final_ratio >= ratio) {
			edr_cur_ratio = (ratio - 1 * 10000) * 10000 / (edr_final_ratio - 1 * 10000);
			if (sdr_nit <= 30)
				edr_cur_ratio = (1 * 10000 + edr_cur_ratio * range - 1 * 10000) * 10000 /
					(scale_max - 1 * 10000);
			sdr_nit = (sdr_nit > 550) ? 550 : sdr_nit;
			sdr_nit = (sdr_nit < 150) ? 150 : sdr_nit;
			edr_cur_nit = 550 - (550 - sdr_nit) * edr_cur_ratio / 10000;
		} else {
			edr_cur_nit = hdr_nit;
		}
	} else {
	...
	IRIS_LOGI("edr_cur_nit:%d ", edr_cur_nit);
...
void iris_sdr2hdr_level_set(u32 level)
{
	...
	if (level != SDR2HDR_Bypass) {
		...
		if (level == SDR2HDR_9) {
			iris_hdr_ai_input_bl(550, false);
			iris_edr_info_init();
		} else
...
void iris_sdr2hdr_update_size2(uint32_t img_width, uint32_t img_height)
{
	...
	/* for TOP win, HDR_IMG_SIZE_2, FILM_ROW_2, FILM_COL_2, TM_VAL_NUM_RATIO_2 */
	{
		...
		IRIS_LOGD("%s(), HDR_IMG_SIZE_2: 0x%08X, FILM_ROW_2: 0x%08X, FILM_COL_2: 0x%08X, TM_VAL_NUM_RATIO_2: 0x%08X",
	...
	/* for NOISE win, NOISE_WIN_COL_2, NOISE_WIN_ROW_2, NOISE_REC_BLK_NUM_2 */
	{
		...
		IRIS_LOGD("%s(), NOISE_REC_BLK_NUM_2: 0x%08X, NOISE_WIN_COL_2: 0x%08X, NOISE_WIN_ROW_2: 0x%08X",
	...
	/* for FLESH DET win, JUDGE_FLESH_12, JUDGE_FLESH_13, JUDGE_FLESH_14, JUDGE_FLESH_15 */
	{
		...
		IRIS_LOGD("%s(), JUDGE_FLESH_12: 0x%08X, JUDGE_FLESH_13: 0x%08X, JUDGE_FLESH_14: 0x%08X, JUDGE_FLESH_15: 0x%08X",
...
int iris_update_backlight_i7(u32 bl_lvl)
{
	...
	char led_pwm1[3] = {MIPI_DCS_SET_DISPLAY_BRIGHTNESS, 0x0, 0x0};
	/*Don't refer HDK, MIPI_DSI_MSG_LASTCOMMAND need replace to 0*/

...
void iris_frc_reg_add_i7(u32 addr, u32 val, bool last)
{
	...
	if (iris_memc_cmd_payload_count >= (IRIS_MEMC_CMD_PAYLOAD_LENGTH - 2)) {
		iris_ocp_write_mult_vals(iris_memc_cmd_payload_count, iris_memc_cmd_payload);
		iris_memc_cmd_payload_count = 0;
		IRIS_LOGI("%s: re_add:%x val:%x last:%d firstly send cmds to avoid payload buffer full", __func__, addr, val, last);
	}
	...
	if (iris_memc_cmd_payload_count < IRIS_MEMC_CMD_PAYLOAD_LENGTH) {
		...
		*payload = val;
		...
		IRIS_LOGE("%s: re_add:%x val:%x last:%d payload buffer length is not enough!", __func__, addr, val, last);
	}
}
...
void iris_fi_reg_set_i7(void)
{
	...
	if (pcfg->memc_info.memc_mode == MEMC_SINGLE_GAME_ENABLE ||
		...
		iris_frc_reg_add_i7(IRIS_FI_ADDR + FI_OSD0_BR, (frc_setting->mv_vres / 2 + 5) << 16 |
			(frc_setting->mv_hres / 2 + 5), 0);
		iris_frc_reg_add_i7(IRIS_FI_ADDR + FI_OSD0_TL, (frc_setting->mv_vres / 2 - 5) << 16 |
			(frc_setting->mv_hres / 2 - 5), 0);
	}
...
void iris_demo_wnd_set_i7(void)
{
	...
	iris_memc_cmd_payload_init_i7();
	iris_frc_reg_add_i7(IRIS_FI_ADDR + FI_DEMO_MODE_CTRL, 0x4000 |
			pcfg->frc_label << 7 |
			pcfg->demo_border << 4 |
			pcfg->demo_enable << 1 |
			pcfg->demo_color_en
	...
	iris_frc_reg_add_i7(IRIS_FI_ADDR + FI_DEMO_YUV_VALUE, pcfg->demo_color, 0);
	iris_frc_reg_add_i7(IRIS_FI_ADDR + FI_SHDW_CTRL, 1, 1);
	iris_memc_cmd_payload_send_i7();
}

...
void oplus_cam_eeprom(struct cam_eeprom_ctrl_t *e_ctrl)
{
	if (e_ctrl->io_master_info.master_type == CCI_MASTER) {
		if (e_ctrl->io_master_info.cci_client->sid==0x3A) {
			oplus_cam_eeprom_dw9786(e_ctrl);
		}
	}
}

int oplus_cam_eeprom_dw9786(struct cam_eeprom_ctrl_t *e_ctrl)
{
	int                                rc = 0;
...
int32_t EEPROM_CommonWrite(struct cam_eeprom_ctrl_t *e_ctrl,
	...
	if (cam_write_eeprom->isWRP == 0x01) {
		...
		if ((strcmp(cam_write_eeprom->eepromName, "imx766_gt24p256c_main") == 0)
		    || (strcmp(cam_write_eeprom->eepromName, "gt24p128f_imx355") == 0)){
			i2c_reg_array.reg_addr = 0xE000;
			i2c_reg_array.reg_data = 0xA2;
		} else if ((strcmp(cam_write_eeprom->eepromName, "imx766_gt24p256c_tele") == 0)
                 ||(strcmp(cam_write_eeprom->eepromName, "imx890_gt24p256c_tele") == 0)
                 ||(strcmp(cam_write_eeprom->eepromName, "gt24p128g2csli_s5kjn5_tele") == 0)){
			i2c_reg_array.reg_addr = 0xE000;
		 ...
		 ||(strcmp(cam_write_eeprom->eepromName, "imx882_p24c128e") == 0)){
			...
			i2c_reg_array.reg_data = 0x06;
                  ...
                  ||(strcmp(cam_write_eeprom->eepromName, "p24c128e_imx581") == 0)){
			i2c_reg_array.reg_addr = 0x8000;
			i2c_reg_array.reg_data = 0x00;
		} else {

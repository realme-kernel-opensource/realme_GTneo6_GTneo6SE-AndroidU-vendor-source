...
static const char *cali_str[CALI_STR_MAX] = {"none", "start_cali", "cali_re",
	...
	"cali_q_f0", "show_q_f0", "get_re_range",  "get_f0_range"
};
...
static int aw_cali_svc_get_devs_f0_range(struct aw_device *aw_dev,
						uint32_t *data_buf, int num)
{
	int ret, cnt = 0;
	struct list_head *dev_list = NULL;
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			/*buf[0]:pri_l_f0_min, buf[1]:pri_l_f0_min, buf[2]:pri_r_f0_min, buf[3]:pri_r_f0_min*/
			data_buf[F0_MIN_FLAG + local_dev->channel * 2] =
				local_dev->f0_min;
			data_buf[F0_MAX_FLAG + local_dev->channel * 2] =
				local_dev->f0_max;
			cnt++;
		} else {
			aw_dev_err(local_dev->dev, "channel num[%d] overflow buf num[%d]",
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	uint32_t re_value[AW_DEV_RE_RANGE] = { 0 };
	uint32_t f0_value[AW_DEV_F0_RANGE] = { 0 };
	...
	case CALI_STR_SHOW_F0_RANGE: {
		ret = aw_cali_svc_get_devs_f0_range(aw_dev, f0_value, AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_err(aw_dev->dev, "get f0 range failed");
			return -EINVAL;
		}
		for (i = 0; i < ret; i++)
			len += snprintf(local_buf + len, sizeof(local_buf) - len,
				"%s:f0_min:%d f0_max:%d ",
				ch_name[i], f0_value[F0_MIN_FLAG + i * F0_RANGE_NUM],
				f0_value[F0_MAX_FLAG + i * F0_RANGE_NUM]);
...
static void aw_cali_parse_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(np, "f0-enable", &desc->cali_f0_en);
	if (ret < 0) {
		desc->cali_f0_en = false;
		aw_dev_info(aw_dev->dev, "read f0-enable failed, default disable");
	} else {
		aw_dev_info(aw_dev->dev, "f0-enableg = %d", desc->cali_f0_en);
	}
...
int aw_cali_parse_re_dt(struct aw_device *aw_dev)
{
	...
	aw_dev_info(aw_dev->dev, "re min: %d, re max: %d, re default: %d",
					aw_dev->re_min, aw_dev->re_max, aw_dev->cali_desc.cali_re);
	return 0;
}

int aw_cali_parse_f0_dt(struct aw_device *aw_dev)
{
	int ret;
	...
	aw_dev_info(aw_dev->dev, "f0 min: %d, f0 max: %d, f0 default: %d",
					aw_dev->f0_min, aw_dev->f0_max, aw_dev->cali_desc.cali_f0);
	return 0;
}
...
int aw882xx_cali_init(struct aw_cali_desc *cali_desc)
{
	...
	ret = aw_cali_parse_f0_dt(aw_dev);

...
static void oplus_comm_smooth_to_soc(struct oplus_chg_comm *chip, bool force)
{
	...
	for (i = reserve_soc - 1; i >= 0; i--) {
		...
		if (soc < soc_jump_table[reserve_soc][i]) {
			temp_soc = (int)((chip->batt_rm * 10000 / chip->batt_fcc ) / 85) + 1;
			chg_err("temp_soc =%d gauge=%d =%d =%d \r\n",temp_soc,(int)((chip->batt_rm * 10000 / chip->batt_fcc ) / 85),chip->batt_rm,chip->batt_fcc);
		}
		if (soc >= soc_jump_table[reserve_soc][i]) {
			temp_soc = soc + 3;
			break;

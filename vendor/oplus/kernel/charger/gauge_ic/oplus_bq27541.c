...
static int gauge_get_calib_time(
	int *dod_calib_time, int *qmax_calib_time, int gauge_index)
{
	int ret = 0;
	struct chip_bq27541 *chip;
	int *dod_calib_time_pre;
	int *qmax_calib_time_pre;
	static int main_dod_calib_time_pre = 0;
	static int sub_dod_calib_time_pre = 0;
	static int main_qmax_calib_time_pre = 0;
	static int sub_qmax_calib_time_pre = 0;

	if (!gauge_ic)
		return -1;

	mutex_lock(&gauge_ic->calib_time_mutex);
	if (!gauge_index) {
		chip = gauge_ic;
		dod_calib_time_pre = &main_dod_calib_time_pre;
		qmax_calib_time_pre = &main_qmax_calib_time_pre;
	} else {
		chip = sub_gauge_ic;
		dod_calib_time_pre = &sub_dod_calib_time_pre;
		qmax_calib_time_pre = &sub_qmax_calib_time_pre;
	}

	if (!dod_calib_time || !qmax_calib_time || !chip) {
		mutex_unlock(&gauge_ic->calib_time_mutex);
		return -1;
	}
	...
	if (!oplus_vooc_get_allow_reading() || atomic_read(&chip->suspended) == 1 ||
		...
		mutex_unlock(&gauge_ic->calib_time_mutex);
		*dod_calib_time = *dod_calib_time_pre;
		*qmax_calib_time = *qmax_calib_time_pre;
		return 0;
	}
	...
	*dod_calib_time_pre = *dod_calib_time;
	*qmax_calib_time_pre = *qmax_calib_time;

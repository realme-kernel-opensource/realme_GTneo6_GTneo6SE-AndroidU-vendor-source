...
void get_task_time_handler(void *data, struct task_struct *task, struct rq *rq, int tick, int ticks)
{
	int idx;
	...
	ktask_overtime[cpu].task[idx].exec_start = jiffies_to_nsecs(jiffies - ticks);
	ktask_overtime[cpu].task[idx].end = jiffies;
	ktask_overtime[cpu].task[idx].delta = jiffies_to_nsecs(ticks);

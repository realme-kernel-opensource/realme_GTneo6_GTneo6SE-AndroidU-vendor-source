...
static void account_task_time_handler(void *data, struct task_struct *p, struct rq *rq, int user_tick, int ticks)
{
	int idx;
	int cpu = task_cpu(p);
	...
	ktask_cpustat[cpu].cpustat[idx].begin = jiffies - cputime_one_jiffy * ticks;

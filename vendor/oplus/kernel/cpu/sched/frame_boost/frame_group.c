...
static void update_group_exectime(struct frame_group *grp, int group_id)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE) && (group_id == DEFAULT_FRAME_GROUP_ID)) {
	...
	unsigned int flags)
{
	...
	if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE) && (grp != &sf_composition_group)) {
		val_systrace_c(vutil, "vutil");
		val_systrace_c(use_vutil, "use_vutil");
	}

	if (unlikely(sysctl_frame_boost_debug & DEBUG_FTRACE)) {
		trace_printk("flags=%u wallclock=%llu window_start=%llu timeline=%llu prev_putil=%lu curr_util=%lu(curr_exec_util=%lu) vutil=%lu use_vutil=%d rt_grp=%d\n",
	...
	u64 wallclock, bool default_part, struct frame_group *grp)
{
	...
	if (likely(delta_wc_ws >= running)) {
		...
		if (unlikely(sysctl_frame_boost_debug & DEBUG_SYSTRACE) && default_part) {
			val_systrace_c(grp->curr_window_exec, "curr_window_exec");
			val_systrace_c(get_frame_putil(grp->curr_window_scale, grp->frame_zone),
				"curr_window_util");
		}

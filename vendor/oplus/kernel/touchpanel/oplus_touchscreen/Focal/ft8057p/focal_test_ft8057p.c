...
static int fts_open_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	/* get open data */
	byte_num = node_num * 2;
	ret = read_mass_data(FACTORY_REG_OPEN_ADDR, byte_num, ts_data->open_data);
	if (ret) {
...
static int fts_lcdnoise_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	lcdnoise = ts_data->lcdnoise_data;
...
*/
	...
	ts_data->fts_autotest_offset->node_valid[0 * ts_data->hw_res->RX_NUM] = 0;
	ts_data->fts_autotest_offset->node_valid[17 * ts_data->hw_res->RX_NUM] = 0;
	if (FTS_THR_DEBUG) {
...
#define FTS_TEST_FW_NAME    "focaltech_test_firmware.bin"
